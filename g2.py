...
def check_validity(tuple):
    ...
    if not (pair11 and pair1_1 and pair_11 and pair_1_1):
        ...
        if not change(index,rowindex,failed_condition):
           return False
        else:
            return True
    return True
...
def s5(row,array,num,num2):
    ...
    if len(result)>1:
        ...
        return -1
def update(rowindex,column,number):

    graph[column][rowindex] = number



def change(index,rowindex,failed_condition):
    # row = graph[i]
    array = graph[index]
    row = graph[rowindex]
    for condition in failed_condition:
        ...
        situation5 = s5(row,array,pair[condition][0]*-1,pair[condition][1])
        if situation1>-1:
            array[situation1] = pair[condition][1]
            # flip(graph)
            update(index,situation1,pair[condition][1])
        elif situation2>-1:
            row[situation2] = pair[condition][0]
            # flip(graph)
            update(rowindex,situation2,pair[condition][0])
        elif situation3>-1:
            row[situation3] = pair[condition][0]
            array[situation3] = pair[condition][1]
            update(rowindex,situation3,pair[condition][0])
            update(index,situation3,pair[condition][1])
        elif situation4 != -1:
            #a temp graph?

            for i in situation4:

                if marked==[] or not ([index,i] in marked[-1] or [i,index] in marked[-1]):
                # if [index,i] in marked:
                    marked.append([index,i])
                    array[i] = array[i]*-1
                    update(index,i,array[i])
                    # flip(graph)
                    for j in checking[0:-1]:
                        if index in j:
                            if check_validity(j):
                                pass
                            else:
            ...
            #a temp graph?
            for i in situation5:
                if marked == [] or not ([rowindex,i] in marked[-1] or [i,rowindex] in marked[-1]):
                # if [index,i] in marked:
                    marked.append([rowindex,i])
                    row[i] = row[i]*-1
                    update(rowindex,i,row[i])
                    # flip(graph)
                    for j in checking[0:-1]:
                        if rowindex in j:
                            if check_validity(j):
